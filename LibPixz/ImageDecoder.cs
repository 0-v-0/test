	...
	internal class ImageDecoder
	{
		...
		internal static Image DecodeImage(BinaryReader reader, ImgInfo imgInfo)
		{
			...
			for (int i = 1; i < arr.Length; i++)
			{
				...
				{
					samplingFactorX = Math.Max(componentMax.samplingFactorX, arr[i].samplingFactorX),
					samplingFactorY = Math.Max(componentMax.samplingFactorY, arr[i].samplingFactorY)
				};
			}
			...
			for (int y = 0; y < imgInfo.height; y++)
			{
				...
				{
					var c = imagen[y, x];
					set.Add(new Color(c.r, c.g, c.b, c.a));
				}
			}
			bmp.Count = set.Count;
